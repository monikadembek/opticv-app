...
## Post-Review Follow-up (found during manual testing)

- **Critical bug found after this review, now fixed:** `apps/opticv-be/src/app/users/users.service.ts:33-37` (`UsersService.upsertUser`) keyed its `upsert` on `where: { email: data.email }`, with `supabaseId` only set in `create`/`update`. After completing an email change, Supabase's `auth.getUser()` returns the same `supabaseId` with the new email. `SupabaseGuard.canActivate` (`apps/opticv-be/src/app/auth/supabase.guard.ts:43`) calls `upsertUser` on every authenticated request, and the upsert failed to find a row by the new email, then attempted to `create` a new row — colliding with the existing row's `supabaseId` unique constraint (`P2002` on `users_supabaseId_key`) and breaking **all** authenticated API requests for the user post-email-change.
  - This directly corresponds to the spec's "Out of scope" note ("the `User.email` column mirrored in the app's own database ... is not addressed by this task — out of scope to reconcile") — in practice this wasn't just an unreconciled mirror column, it was a hard outage for the affected user, since `upsertUser` runs on the `authGuard`-equivalent for every backend request.
  - **Fix:** upsert now keys on `where: { supabaseId: data.supabaseId }` (the stable identifier) and updates `email` in the `update` clause, so the mirrored email self-heals on the next authenticated request after a Supabase email change.
  - Updated `apps/opticv-be/src/app/users/users.service.spec.ts`: adjusted the existing upsert-shape assertion and added a new test ("updates the email of an existing user when it has changed in Supabase") covering the email-change path.
  - **Recommendation:** given this was a functional break in a shared/core auth path (not scoped to Task 86's file list), it should be called out explicitly to the author/reviewer rather than silently folded into this task's diff — consider a follow-up ticket or noting it in the PR description as a related fix, since `users.service.ts` was not part of Task 86's planned scope.
