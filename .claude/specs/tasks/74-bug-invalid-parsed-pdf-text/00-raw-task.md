Task 74: Bug - text parsed from pdf is invalid

Description:
There is an issue with pdf-parse library, if user uploads cv that contains 2 columns and some graphical elements then the parsed text may be in invalid order and when we pass this text to model to extract data an put it in defined data structure we get invalid structured data back. Maybe we should consider omitting the pdf-parse library if it doesn't produce proper results and instead run the extract cv data prompt with the pdf file attached to extract text content and put it in the defined data structure and then save the structured data in db.

Below there is a plan I already discussed with Claude:
